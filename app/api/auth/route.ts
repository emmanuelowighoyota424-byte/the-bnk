...
function generateAccountNumber(): string { return `9${Math.floor(100000000 + Math.random() * 900000000)}` }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'signup') {
      ...
      return NextResponse.json({ message: 'User created successfully', userId: result.user.id, accountNumber: result.account.accountNumber, maskedAccountNumber: `****${result.account.accountNumber.slice(-4)}`, authenticated: false }, { status: 201 })
    }

    if (action === 'login') {
      if (!password) return NextResponse.json({ error: 'Password is required' }, { status: 400 })
      const user = await prisma.user.findUnique({ where: { email } })
      if (!user || user.status !== 'active' || !(await verifyPassword(password, user.passwordHash))) return NextResponse.json({ error: 'Invalid credentials' }, { status: 401 })
