...
type MovementType = 'TRANSFER' | 'DEPOSIT' | 'WITHDRAWAL' | 'ADMIN_ADJUSTMENT';
...
export async function transferFunds(userId: string, input: { senderAccountId: string; recipientAccountNumber: string; amount: string; description?: string; idempotencyKey: string }, meta: { ip?: string; ua?: string }) {
  ...
  const result = await serializableTransaction(async (tx) => {
    ...
    const senderUser = await tx.user.findUnique({ where: { id: userId }, select: { transferBlocked: true, transferBlockMessage: true } });
    if (senderUser?.transferBlocked) throw new Error(senderUser.transferBlockMessage || 'Transfers and send money are currently blocked on your account.');
...
export async function adjustCustomerBalance(adminId: string, input: { userId: string; accountId: string; amount: string; direction: 'credit' | 'debit'; reason: string }) {
  const amount = money(input.amount);
  if (amount.lte(0)) throw new Error('Amount must be greater than zero');
  ...
  const result = await serializableTransaction(async (tx) => {
    ...
    const posted = await postJournal(tx, {
      ...
      debitTransaction: {
        ...
        description: input.direction === 'credit' ? 'Admin balance funding' : 'Admin balance removal',
      },
      creditTransaction: {
        accountId: input.direction === 'credit' ? account.id : clearing.id,
        userId: input.direction === 'credit' ? input.userId : clearing.userId,
