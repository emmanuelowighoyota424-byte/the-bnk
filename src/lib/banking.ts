...
/**
 * Authoritative double-entry posting primitive.
 *
 * A single journalId identifies the complete financial movement. The two
 * immutable ledger rows are the debit and credit sides of that journal.
 * The caller must mutate the affected account balances in the same database
 * transaction; this function refuses invalid/unbalanced postings.
 */
async function postJournal(
  ...
  const lines = await tx.ledgerEntry.findMany({
    where: { transactionId: { in: [debitTx.id, creditTx.id] } },
...
export async function transferFunds(
  userId: string,
  input: { senderAccountId: string; recipientAccountNumber: string; amount: string; description?: string; idempotencyKey: string },
...
export async function approveDeposit(adminId: string, depositId: string, reason?: string) {
  const result = await prisma.$transaction(async (tx) => {
    ...
    await postJournal(tx, {
      ...
      debitTransaction: { accountId: clearing.id, userId: clearing.userId, description: 'Deposit clearing' },
