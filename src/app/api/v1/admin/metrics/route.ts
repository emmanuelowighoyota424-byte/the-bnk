...
export async function GET(request: NextRequest) {
  try {
    ...
    const [totalUsers,activeUsers,totalAccounts,totalBalance,pendingKyc,todayTxs,openDisputes,openAlerts] = await Promise.all([
      prisma.user.count(), prisma.user.count({ where: { status:'active' } }), prisma.account.count(),
      prisma.account.aggregate({ _sum: { balance: true } }), prisma.kYCDocument.count({ where: { verificationStatus:'pending' } }),
      prisma.transaction.count({ where: { createdAt: { gte: new Date(Date.now()-24*60*60*1000) } } }),
      prisma.dispute.count({ where: { status: { not:'closed' } } }), prisma.fraudAlert.count({ where: { status:'open' } })
    ]);
    return successResponse({ users:{total:totalUsers,active:activeUsers}, accounts:{total:totalAccounts,totalBalance:totalBalance._sum.balance||0}, kyc:{pending:pendingKyc}, transactions:{today:todayTxs}, disputes:{open:openDisputes}, fraud:{openAlerts} });
