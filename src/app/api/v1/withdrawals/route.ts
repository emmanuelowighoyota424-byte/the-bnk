...
export async function POST(req: NextRequest) {
  ...
  try { return successResponse(await createWithdrawal(userId, v.data), 201); }
...
export async function GET(req: NextRequest) {
  const userId = req.headers.get('x-user-id');
  if (!userId) return unauthorizedResponse();
  return successResponse(await prisma.withdrawal.findMany({ where: { userId }, orderBy: { createdAt: 'desc' }, take: 50 }));
}
