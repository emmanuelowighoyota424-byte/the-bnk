...
export default function TransferPage() {
  ...
  return (
    <>
      <BankNav />
      <main className="min-h-[calc(100vh-4rem)] bg-slate-50 pb-24 lg:pb-10">
        <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
          <div className="mb-7 flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-indigo-700">Money</p>
            ...
            <Link href="/transactions" className="hidden shrink-0 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 sm:inline-flex">Activity</Link>
          </div>
              ...
              {steps.map((label, index) => (
                <div key={label} className="flex min-w-0 flex-1 items-center">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${index <= step ? 'bg-indigo-700 text-white' : 'bg-slate-100 text-slate-400'}`} aria-current={index === step ? 'step' : undefined}>
                      {index < step ? '✓' : index + 1}
                    </span>
                    <span className={`hidden truncate text-xs font-semibold sm:block ${index === step ? 'text-slate-950' : 'text-slate-400'}`}>{label}</span>
                  </div>
                  {index < steps.length - 1 && <span className={`mx-2 h-px flex-1 ${index < step ? 'bg-indigo-600' : 'bg-slate-200'}`} aria-hidden="true" />}
                </div>
              ))}
            </div>
